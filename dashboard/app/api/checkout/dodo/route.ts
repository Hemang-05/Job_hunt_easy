...
// Creates a Dodo Payments checkout link
// - India: One-time lifetime payment (/payments)
// - Global: Monthly subscription (/subscriptions)
// ============================================================
...
export async function POST(req: NextRequest) {
  try {
    ...
    const productId = isIndia
      ? process.env.NEXT_PUBLIC_DODO_PRODUCT_ID_IND  // one-time lifetime INR product
      : process.env.NEXT_PUBLIC_DODO_PRODUCT_ID       // monthly subscription USD product

    const origin = req.headers.get('origin')
    const appUrl = origin || process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'

    if (!apiKey || !productId) {
      console.error('[Checkout] Missing env vars — apiKey:', !!apiKey, 'productId:', !!productId)
    ...
    const response = await fetch(`${dodoBaseUrl}${endpoint}`, {
      method: 'POST',
      headers: {
