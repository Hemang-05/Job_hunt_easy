...
import { Check, Zap, X, ArrowLeft, Infinity } from 'lucide-react'
...
export default function PricingClient({ initialUser, initialPlan, userCountry }: PricingClientProps) {
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a] text-white flex flex-col items-center justify-center px-4 py-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full max-w-3xl">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-8 flex flex-col">
          ...
          <button
            ...
            className="w-full rounded-xl py-3 text-sm font-semibold bg-white/5 text-gray-500 border border-white/10 disabled:cursor-not-allowed enabled:cursor-pointer enabled:hover:bg-white/10 enabled:text-white"
          >
        ...
        <div className="rounded-2xl border border-indigo-500/40 bg-indigo-500/[0.07] p-8 flex flex-col relative overflow-hidden">
          ...
          <div className="mb-6">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="bg-indigo-500/20 text-indigo-300 text-xs px-2 py-0.5 rounded-full border border-indigo-500/30">
                {isIndia ? 'Best value' : 'Most popular'}
              </span>
            </div>

            {/* Price display — different for India vs Global */}
            {isIndia ? (
              <>
                <div className="flex items-end gap-1">
                  <span className="text-4xl font-bold">₹299</span>
                  <span className="text-gray-400 mb-1 line-through text-sm ml-1">₹999</span>
                </div>
                <div className="flex items-center gap-2 mt-1">
                  <span className="bg-emerald-500/20 text-emerald-400 text-xs px-2 py-0.5 rounded-full border border-emerald-500/30 font-medium">
                    Pay what you want · min ₹299
                  </span>
                </div>
                <p className="text-gray-400 text-sm mt-2 flex items-center gap-1">
                  <Infinity className="w-3.5 h-3.5" /> Lifetime access · one-time payment
                </p>
              </>
            ) : (
              <>
          ...
          <button
            ...
            disabled={loading}
            className="w-full rounded-xl py-3 text-sm font-semibold bg-indigo-500 text-white flex items-center justify-center gap-2 hover:bg-indigo-600 transition-colors disabled:opacity-70 disabled:cursor-not-allowed"
          >
      ...
      {/* Footer notes */}
      <div className="mt-16 max-w-lg w-full space-y-3 text-sm text-gray-500 text-center">
        <p>✦ Your plan upgrades instantly after payment</p>
        {isIndia
          ? <p>✦ One-time payment · access never expires</p>
          : <p>✦ Cancel anytime from your account settings</p>
        }
        <p>✦ Free plan resets daily at midnight UTC</p>
      </div>
